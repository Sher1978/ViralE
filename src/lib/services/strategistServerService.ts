  ...
  async getStrategistSystemPrompt(userId: string, locale: string = 'en'): Promise<string> {
    ...
    return `
      ...
      1. СКАНИРОВАНИЕ И ИНТЕРВЬЮ STORYBRAND:
         ...
         Если они отсутствуют, проведи короткое интервью. Если присутствуют, переходи к шагу 2.
         
      2. ГЕНЕРАЦИЯ 3 ИДЕЙ (ЛЕСТНИЦА ХАНТА):
         Когда пользователь просит идеи для видео (или если ДНК уже собрано и подтверждено), сгенерируй ровно 3 идеи для видео, используя Лестницу Ханта (выбери 3 разные ступени осведомленности). 
