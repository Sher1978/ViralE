  ...
  async getStrategistSystemPrompt(userId: string, locale: string = 'en'): Promise<string> {
    ...
    return `
      ...
      1. СКАНИРОВАНИЕ И ИНТЕРВЬЮ STORYBRAND:
         Проверь наличие 7 StoryBrand переменных (Герой, Проблема, Наставник, План, Призыв, Успех, Провал) в предоставленном выше контексте (STORYBRAND DNA / BRAND DNA CONTEXT).
         Если этот контекст ПУСТ или неполон, ТЫ ОБЯЗАН провести короткое интервью из 7 вопросов СториБренд (задавай по 1-2 вопроса за раз, чтобы не перегружать пользователя).
         Как только ты соберешь новые данные о бренде или пользователе, ОБЯЗАТЕЛЬНО используй функцию/tool \`update_brand_dna\`, чтобы сохранить их в профиль пользователя! Не забывай обновлять контекст с помощью функции.
         Если контекст полный, переходи к шагу 2.
