...
export default function StoryboardPage() {
    ...
    async function loadData() {
      ...
      try {
        ...
        if (!proj || !ver) {
          ...
          setProject(proj);
          setVersion(ver);
          // Auto-trigger generation if data is missing
          if (!ver.storyboard_data || (ver.storyboard_data as any).scenes?.length === 0) {
            setIsLoading(false); // To prevent infinite loop if we decide not to auto-trigger
            // But let's auto-trigger for better UX
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
        ...
        <button 
          onClick={() => router.push(`/${locale}/projects/new/script`)}
  ...
  const scenes = scriptData ? [
    { label: t('tagHook') || 'HOOK', text: scriptData.hook },
    { label: t('tagStory') || 'STORY', text: scriptData.story },
    { label: t('tagCTA') || 'CTA', text: scriptData.cta }
  ] : [];
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-24">
      ...
      {(!version?.storyboard_data || (version.storyboard_data as any).scenes?.length === 0) && (
        ...
        {scenes.map((scene, index) => (
          <div key={index} className="space-y-3">
            <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black tracking-[0.2em] text-white/30 uppercase">
                  {t('frameLabel')} {index + 1}: {scene.label}
                </span>
                <span className="px-1.5 py-0.5 rounded bg-orange-500/20 text-[8px] font-black text-orange-400 uppercase tracking-tighter">
                  {t('selectedLabel')}
                </span>
              </div>
              <button className="flex items-center gap-1.5 text-[9px] font-bold text-orange-400 uppercase tracking-widest hover:text-orange-300 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div
                className="aspect-square rounded-2xl bg-white/5 border border-white/10 overflow-hidden flex items-center justify-center group cursor-pointer"
              >
                <div className="text-center space-y-2 opacity-20 group-hover:opacity-100 transition-all">
                  ...
                  <p className="text-[8px] font-bold text-white/40 uppercase tracking-widest">Variant B</p>
                </div>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-inherit backdrop-blur-xl border-t border-white/5 z-50">
        <div className="flex gap-3">
          <Link href={`/${locale}/projects/new/script?projectId=${projectIdParam}&versionId=${versionIdParam}`} className="flex-1">
          ...
          <Link href={`/${locale}/projects/new/production?projectId=${projectIdParam}&versionId=${versionIdParam}`} className="flex-[2]">
