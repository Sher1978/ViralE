...
export default function StudioPage() {
  ...
  const [activeTab, setActiveTab] = useState<'layout' | 'assets' | 'settings'>('layout');
  ...
  const regenerateSegment = async (segmentId: string) => {
    ...
    try {
      ...
      // Logic for polling completion would go here or via real-time Supabase subscription
  ...
  const triggerFinalRender = async () => {
    ...
    try {
      ...
      router.push(`/${locale}/projects/${projectId}?tab=delivery`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#05050a] text-white overflow-hidden font-sans">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden relative bg-[#05050a]">
          ...
          <div className="h-72 border-t border-white/5 bg-[#0a0a14]/80 backdrop-blur-3xl p-6 flex flex-col z-20">
            ...
            {/* Scrolling Timeline */}
            <div className="flex-1 overflow-x-auto flex gap-3 pb-2 scrollbar-thin scrollbar-thumb-white/10 px-2">
              {manifest?.segments.map((segment, index) => (
                <div 
                  key={segment.id}
                  onClick={() => setSelectedSegmentId(segment.id)}
                  className={`flex-none w-48 rounded-2xl border transition-all cursor-pointer overflow-hidden flex flex-col group/item ${
                ...
                >
                  ...
                  <div className="p-3 bg-black/20">
                    <p className="text-[9px] font-bold text-white/40 line-clamp-1 uppercase tracking-tighter mb-1">
                      {segment.type}
                    </p>
        ...
        <aside className="w-80 border-l border-white/5 bg-[#0a0a14] flex flex-col overflow-y-auto z-30">
          {selectedSegment ? (
            <div className="p-8 space-y-8 animate-fade-in">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-white/30 tracking-widest ml-1">Prompt Configuration</label>
                  <textarea 
                    className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-xs text-white/60 focus:outline-none focus:border-purple-500/50 min-h-[140px] leading-relaxed resize-none transition-all"
                    defaultValue={selectedSegment.prompt}
                  />
                </div>
      ...
      {/* Viral Strategist AI Chat */}
      {manifest && currentProfile && (
        <StrategistChat 
          projectId={projectId}
          manifest={manifest}
          setManifest={setManifest}
          activeSegmentId={selectedSegmentId || undefined}
          locale={locale}
        />
      )}
