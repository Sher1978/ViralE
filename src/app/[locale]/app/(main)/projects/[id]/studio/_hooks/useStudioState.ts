...
export function useStudioState(projectId: string, initialManifest: ProductionManifest | null, propARollUrl: string | null) {
  ...
  const [subtitleSize, setSubtitleSize] = useState(46); // Standard 1080p canvas size (+20% increased CapCut/Edits style)
    ...
    async function recoverDraft() {
      ...
      try {
        ...
        if (data) {
          ...
          if (data.subtitleSize) setSubtitleSize((data.subtitleSize && data.subtitleSize >= 25) ? data.subtitleSize : 46);
