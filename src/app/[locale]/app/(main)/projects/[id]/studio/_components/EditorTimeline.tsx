...
export const EditorTimeline: React.FC<EditorTimelineProps> = ({
  ...
  // Helper to convert screen X (clientX) to timeline time (seconds)
  const getClickTime = (clientX: number) => {
    if (!contentRef.current) return 0;
    const rect = (contentRef.current as any).getBoundingClientRect();
  ...
  const handleScroll = () => {
    ...
    if (isPlaying) {
      // Pause playback if the user scrolls manually
      onScrollStart?.();
      return;
    }
  ...
  return (
    <div className="w-full bg-[#080808] border-t border-white/[0.06] flex flex-col select-none h-[260px] relative">
      {/* Playhead / Needle (Fixed at viewport center) */}
      <div className="absolute inset-y-0 left-1/2 w-[2px] bg-white z-30 pointer-events-none shadow-[0_0_15px_rgba(255,255,255,0.5)]">
      ...
      <div 
        ...
        onMouseDown={(e) => {
          ...
          if ((e.target as any).closest('.resize-handle') || (e.target as any).closest('.delete-btn')) return;
          onScrollStart?.();
          setIsScrolling(true);
        }}
        ...
        className="flex-1 overflow-x-auto overflow-y-hidden no-scrollbar cursor-cell select-none relative"
      >
        {/* Padded wrapper to allow full-width viewport centering */}
        <div className="relative h-full flex flex-col" style={{ paddingLeft: '50%', paddingRight: '50%' }}>
          
          {/* Inner content wrapper matching actual timeline content width */}
          <div 
            ref={contentRef}
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) {
                onSelectClip?.(null, 'aroll');
              }
            }}
          >
            {/* Track Grid Separators (Dashed Boundaries) */}
            ...
            {/* Ruler Line */}
            <div 
              className="h-10 relative border-b border-white/[0.03] pointer-events-auto"
              onDoubleClick={(e) => {
                const clickTime = getClickTime(e.clientX);
            ...
            {/* Tracks Area */}
            <div 
              className="flex-1 relative bg-black/20 pointer-events-auto"
            ...
            >
              ...
              <div 
                ...
                onPointerDown={(e) => {
                  ...
                  // Drag to scroll timeline manually
              ...
              <div 
                ...
                onClick={(e) => {
                  ...
                  const clickTime = getClickTime(e.clientX);
                  if (clickTime >= 0 && clickTime <= totalDuration) {
                    setWbPlaceholderTime(clickTime);
                  }
                }}
              >
