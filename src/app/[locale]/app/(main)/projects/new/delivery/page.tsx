  ...
  const handleClientRender = async (ver: ProjectVersion) => {
    ...
    try {
      ...
      if (!hasBrolls) {
        ...
        const exitCodeFast = await ffmpeg.exec([
          '-i', currentInput,
          '-vf', vfFilter,
        ...
        if (exitCodeFast !== 0) throw new Error('FFmpeg processing failed');
        ...
        const exitCodeScale = await ffmpeg.exec([
          '-i', currentInput,
          '-vf', scale,
          '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '28', '-threads', '1',
          '-c:a', 'aac', '-b:a', '128k',
        ...
        if (exitCodeScale !== 0) throw new Error('FFmpeg scale failed');
